...
Функция Удалить(УдалитьПодзадачи = Ложь) Экспорт
...
Функция ПолучитьСоотРеализованыхЗапросов()
	...
	СтркПараметрыAPI = Новый Структура(СтрокаПоляПараметровAPI, СтркТипЗапроса.удалить, СтрокаЗапросаREST, СтркПараметрыШаблонаREST, СтркПараметрыЗапросаREST);
...
Функция ПолучитьОтветJSONRest(АдресСистемы, КукиДоступа, СтрктRESTЗапроса) Экспорт
	...
	ИначеЕсли СтрктRESTЗапроса.ТипЗапросаREST = "DELETE" Тогда
		Результат = КоннекторHTTP.Delete(АдресСистемы + СтрктRESTЗапроса.АдресREST, , СтрктRESTЗапроса.ПараметрыЗапросаREST, Новый Структура("Заголовки", Заголовки)).Текст();
	Иначе
