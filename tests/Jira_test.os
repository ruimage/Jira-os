...
// Процедура ТестДолжен_ПолучитьПараметрыДоступа() Экспорт
...
// 	Результат = КлассJira.ПолучитьПараметрыДоступа(СтрокаЗапросаJQL, МассивИменПолейОтбора, 1);
...
Процедура ТестДолжен_ПроверитьСоздатьПроблему() Экспорт
	...
	РезультатОбработки = Jira.СоздатьПроблему(ЭталонныйПакетJSON);
	Ожидаем.Что(РезультатОбработки).ИмеетТип("Структура").Заполнено();
	Ожидаем.Что(РезультатОбработки.id).ИмеетТип("Строка").Заполнено();
	Ожидаем.Что(СтрНачинаетсяС(РезультатОбработки.key,"TEST")).ИмеетТип("Булево").Равно(Истина);
